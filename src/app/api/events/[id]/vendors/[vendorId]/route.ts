...
// Handles edit and delete for a single vendor.
//
// PATCH /api/events/[id]/vendors/[vendorId]
//   Updates vendor fields including staffCount.
//
...
// Verifies the event belongs to the authenticated planner
...
// Verifies the vendor belongs to this event (prevents cross-event access)
async function checkVendorOwner(vendorId: string, eventId: string) {
  return prisma.vendor.findFirst({ where: { id: vendorId, eventId }, select: { id: true } })
}
...
export async function PATCH(
  ...
  try {
    ...
    const { name, contactName, email, phone, role, notes, staffCount, canOverrideCapacity } = body
