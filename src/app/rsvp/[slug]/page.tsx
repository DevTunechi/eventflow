...
export default function RSVPPage() {
  ...
  // Meals
  const [mealSelections, setMealSelections] = useState<Record<string, string>>({}) // category → menuItemId
  ...
  const submitRsvp = async ({ skipMeals = false, otpVerified = false } = {}) => {
    ...
    try {
      const meals = skipMeals ? [] : Object.values(mealSelections).map(menuItemId => ({ menuItemId, quantity: 1 }))
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .rsvp-meal-item:hover{border-color:rgba(180,140,60,0.3);background:rgba(180,140,60,0.04)}
        ...
        <div className="rsvp-card">
          ...
          {step === "form" && (
            <>
              ...
              {/* Tier selection — only show if no tier pre-assigned and multiple tiers exist */}
          ...
          {step === "meals" && (
            <>
              ...
              <p style={{ fontSize:"0.78rem", color:"rgba(240,236,228,0.45)", marginBottom:"1.25rem", lineHeight:1.6 }}>
                Select one option per course. Your selections will be prepared before the event.
              </p>
                    ...
                    return (
                      <div key={item.id} className={`rsvp-meal-item${selected ? " selected" : ""}`}
                        ...
                        <div>
                          ...
                          {item.description && <div style={{ fontSize:"0.72rem", color:"rgba(240,236,228,0.45)", marginTop:"0.15rem" }}>{item.description}</div>}
                        </div>
                      </div>
                    )
                  })}
