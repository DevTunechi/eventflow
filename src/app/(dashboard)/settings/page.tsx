...
// Mobile-first — full width on mobile, sidebar layout on desktop 700px+
...
export default function SettingsPage() {
  ...
  const [notifPrefs, setNotifPrefs] = useState({
    rsvpConfirmed: true, rsvpDeclined: false,
    guestCheckedIn: false, giftReceived: true,
    paymentSuccess: true, paymentFailed: true,
  })
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const r = await fetch("/api/settings/profile", { headers: getAuthHeaders() })
        if (r.ok) {
          const d = await r.json()
          setProfile(d.user)
          setForm({ name: d.user.name ?? "", phone: d.user.phone ?? "", businessName: d.user.businessName ?? "", businessLogo: d.user.businessLogo ?? "" })
          setLogoPreview(d.user.businessLogo ?? null)
        }
      } catch { /* silent */ }
      try {
        const r = await fetch("/api/payments/billing", { headers: getAuthHeaders() })
        if (r.ok) {
          const d = await r.json()
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const res = await fetch("/api/settings/profile", {
        ...
        body: JSON.stringify({ name: form.name.trim() || null, phone: form.phone.trim() || null, businessName: form.businessName.trim() || null, businessLogo: form.businessLogo || null }),
      })
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true)
    try {
      await fetch("/api/payments/cancel", { method: "POST", headers: getAuthHeaders() })
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Tab nav — always horizontal, scrollable ── */
        .st-tabs {
          display: flex;
          flex-direction: row;
          overflow-x: auto;
        ...
        /* ── Form fields ── */
        .st-field { margin-bottom: 0.875rem; width: 100%; }
        .st-label {
          display: block;
          font-size: 0.63rem;
          color: var(--text-3);
        ...
        /* 2-col on wider screens, 1-col on mobile */
        .st-row2 { display: grid; grid-template-columns: 1fr; gap: 0.75rem; width: 100%; }
        @media(min-width:500px) { .st-row2 { grid-template-columns: 1fr 1fr; } }

        /* ── Logo upload ── */
        .st-logo-row { display: flex; align-items: center; gap: 0.875rem; margin-bottom: 0.875rem; flex-wrap: wrap; }
        .st-logo-box {
        ...
        .st-logo-btns { display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center; }
        .st-logo-hint { font-size: 0.63rem; color: var(--text-3); margin-top: 0.25rem; }
        ...
        .st-btn-gold:hover:not(:disabled) { background: #c9a050; }
        ...
        .st-btn-red {
          ...
          color: rgba(239,68,68,0.7); font-family: 'DM Sans', sans-serif;
          font-size: 0.75rem; font-weight: 500;
        ...
        .st-info-row {
          ...
          font-size: 0.78rem; gap: 0.75rem; width: 100%;
        }
        ...
        .st-plan-badge {
          ...
          padding: 0.2rem 0.575rem; border-radius: 99px;
          font-size: 0.62rem; font-weight: 500; letter-spacing: 0.05em;
          border: 1px solid rgba(180,140,60,0.35);
          background: rgba(180,140,60,0.08); color: var(--gold);
        }
        ...
        /* ── Plan option cards ── */
        .st-plan-options { display: flex; flex-direction: column; gap: 0.625rem; margin-top: 1rem; }
        @media(min-width:500px) { .st-plan-options { flex-direction: row; } }
        .st-plan-opt { flex: 1; border: 1.5px solid var(--border); padding: 1rem; min-width: 0; }
        .st-plan-opt.current { border-color: var(--gold); background: rgba(180,140,60,0.04); }
        .st-plan-opt-name  { font-size: 0.68rem; font-weight: 500; letter-spacing: 0.08em; text-transform: uppercase; color: var(--gold); margin-bottom: 0.25rem; }
        .st-plan-opt-price { font-family: 'Cormorant Garamond', serif; font-size: 1.25rem; font-weight: 300; color: var(--text); margin-bottom: 0.5rem; }
        .st-plan-opt-label { width: 100%; padding: 0.45rem; border: 1px solid var(--gold); color: var(--gold); font-family: 'DM Sans', sans-serif; font-size: 0.68rem; text-align: center; background: transparent; letter-spacing: 0.04em; }

        /* ── Top-up rows ── */
        .st-topup-row {
          display: flex; align-items: flex-start; justify-content: space-between;
          padding: 0.625rem 0; border-bottom: 1px solid var(--border); gap: 0.75rem;
        }
        ...
        /* ── Coming soon ── */
        .st-coming { padding: 1.5rem 1rem; text-align: center; border: 1px dashed var(--border); width: 100%; }
        ...
        /* ── Danger ── */
        .st-danger { background: rgba(239,68,68,0.04); border: 1px solid rgba(239,68,68,0.2); padding: 1.125rem; width: 100%; }
        ...
        .st-conn-left { display: flex; align-items: center; gap: 0.625rem; min-width: 0; flex: 1; }
      ...
      <div className="st">
        ...
        {/* ── Tab nav — always horizontal ── */}
        <div className="st-tabs">
          {SECTIONS.map(s => (
            <button
              key={s.key}
              className={`st-tab${section === s.key ? " active" : ""}`}
              onClick={() => setSection(s.key)}
            >
              <span className="st-tab-icon">{s.icon}</span>
              {s.label}
            </button>
          ))}
        </div>
        ...
        {section === "profile" && (
          <div className="st-card">
            ...
            <div className="st-field">
              ...
              <div className="st-logo-row">
                <div className="st-logo-box">
                  {logoPreview
                    ? <Image src={logoPreview} alt="Logo" width={56} height={56} style={{ objectFit:"cover", width:"100%", height:"100%" }} unoptimized />
                ...
                <div>
                  ...
                  <div className="st-logo-hint">JPEG, PNG or WEBP · Max 2MB</div>
                </div>
              </div>
            </div>
            ...
            <div className="st-field">
              ...
              <div style={{ fontSize:"0.63rem", color:"var(--text-3)", marginTop:"0.35rem" }}>
          ...
          <>
            <div className="st-card">
              ...
              <div className="st-info-row">
                ...
                <span className="st-plan-badge">{PLAN_LABELS[currentPlan] ?? currentPlan}</span>
              </div>
              ...
              <div className="st-plan-options">
                {[
                  ...
                  <div key={p.key} className={`st-plan-opt${currentPlan === p.key ? " current" : ""}`}>
                    ...
                    {currentPlan === p.key
                      ? <div className="st-plan-opt-label">Current plan</div>
                      : p.key === "free"
                      ? <div style={{ fontSize:"0.67rem", color:"var(--text-3)" }}>Downgrade by cancelling</div>
                      : currentPlan === "pro" && p.key === "starter"
                      ? <div style={{ fontSize:"0.67rem", color:"var(--text-3)" }}>Cancel Pro to downgrade</div>
                      : <button className="st-btn-gold" style={{ width:"100%", marginTop:"0" }}
                          onClick={() => handleUpgrade(p.key as "starter"|"pro")} disabled={!!upgrading}>
              ...
              {currentPlan !== "free" && billing?.status === "ACTIVE" && (
                <div style={{ marginTop:"1rem" }}>
                  <button className="st-btn-ghost" style={{ color:"rgba(239,68,68,0.6)", borderColor:"rgba(239,68,68,0.2)" }}
            ...
            <div className="st-card">
              ...
              <div style={{ fontSize:"0.68rem", color:"var(--text-3)", marginTop:"0.875rem", lineHeight:1.6 }}>
                Purchase top-ups from your event page or the{" "}
                <Link href="/dashboard/pricing" style={{ color:"var(--gold)", textDecoration:"none" }}>pricing page</Link>.
              </div>
            </div>
          </>
        )}
        ...
        {section === "notifications" && (
          <div className="st-card">
            ...
            <p style={{ fontSize:"0.72rem", color:"var(--text-3)", marginBottom:"1rem", lineHeight:1.6 }}>
            ...
            {[
              ...
              <div className="st-toggle-row" key={pref.key}>
                ...
                <div className={`st-switch${notifPrefs[pref.key as keyof typeof notifPrefs]?" on":""}`}
            ...
            <div className="st-btn-row">
              <button className="st-btn-gold" onClick={() => { setNotifSaved(true); setTimeout(()=>setNotifSaved(false),2500) }}>
                Save preferences
              </button>
            </div>
          ...
          <>
            <div className="st-card">
              ...
              <div className="st-conn-row">
                <div className="st-conn-left">
                  <span style={{ fontSize:"1.125rem", flexShrink:0 }}>🔵</span>
                  <div style={{ minWidth:0 }}>
                    <div className="st-conn-name">Google</div>
                    <div className="st-conn-email">{profile?.email}</div>
                  </div>
                </div>
            ...
            <div className="st-card">
              ...
              <div className="st-coming">
                ...
                <div className="st-coming-desc">
                  ...
                  No setup needed — available once our Meta registration is complete.
                </div>
              </div>
            </div>
          </>
        ...
        {section === "security" && (
          <>
            <div className="st-card">
              ...
              <p style={{ fontSize:"0.78rem", color:"var(--text-2)", lineHeight:1.7, marginBottom:"1rem" }}>
                Your account is secured via Google OAuth — EventFlow does not store passwords.
                To change your Google password or enable 2-factor authentication, visit your{" "}
            ...
            <div className="st-card">
              ...
              <button className="st-btn-ghost"
                onClick={() => { localStorage.removeItem("ef-session"); window.location.href = "/login" }}>
        ...
        {section === "danger" && (
          <div className="st-danger">
            <div style={{ fontSize:"0.58rem", fontWeight:500, letterSpacing:"0.18em", textTransform:"uppercase", color:"#ef4444", marginBottom:"1rem" }}>
              Danger Zone
            </div>
            <div style={{ fontSize:"0.82rem", fontWeight:500, color:"var(--text)", marginBottom:"0.375rem" }}>
              Delete your account
            </div>
            <p style={{ fontSize:"0.72rem", color:"rgba(240,236,228,0.5)", lineHeight:1.7, marginBottom:"1.25rem" }}>
